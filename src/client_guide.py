...
def routing_message_html() -> str:
    """Второе сообщение: ссылка happ:// (клик открывает / копирование) + шаги в Happ."""
    href = html_module.escape(RU_ROUTING_HAPP_IMPORT, quote=True)
    ...
    return (
        ...
        "Нажмите на ссылку ниже — на телефоне или ПК обычно откроется Happ с готовыми настройками:\n\n"
        f'<a href="{href}">Ссылка с настройками RU</a>\n\n'
        f"{body}"
    )
